...
public class Eriytymismalli {
    ...
    public ArrayList<Piste> haeTyytymattomat() {
        ...
        for (int xMuunnos = -1; xMuunnos <= 1; xMuunnos++) {
            if (xMuunnos + x < 0 || xMuunnos + x >= this.taulukko.getLeveys()) {
                continue;
            }
 
            for (int yMuunnos = -1; yMuunnos <= 1; yMuunnos++) {
                if (yMuunnos + y < 0 || yMuunnos + y >= this.taulukko.getKorkeus()) {
                    continue;
                }
                ...
                if (this.taulukko.hae(xMuunnos + x, yMuunnos + y) == this.taulukko.hae(x, y)) {
                    naapureita++;
                }
            }
        }
        if (naapureita < this.tyytyvaisyysraja) {
